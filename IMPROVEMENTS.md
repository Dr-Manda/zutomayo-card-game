...
**Size:** M · **Status:** ✓ SHIPPED (Session 2, 2026-06-18) — vitest 4.1 installed with a `node`-environment `vitest.config.ts` mirroring the `@/` path alias from tsconfig. 43 tests in `src/lib/__tests__/gameEngine.test.ts` cover every Wave 1 fix as a regression test (1.1 draw counts, 1.2 full set-zone sweep + defense-in-depth occupant-to-abyss, 1.3 zone-C clock exclusion + turn-1 prepClock, 1.4 any-class initial placement + null-battleZone→0-attack, 1.5 empty-selections no-op, 1.6 mulligan draws-before-shuffle, 1.7 exact set counts, 1.9 simultaneous deck-out is a draw + the directional case where P0 deck-out gives P0 the win and not always P2), plus calculateBattle power-cost gating, HP-0 immediate game-over, replaceCharacter slot-A priority, chronos night/day, drawCards deckEmpty bookkeeping, and an immutability check on endTurn/mulligan. Fixtures are built via local `buildCard`/`buildState` helpers — no dependency on cards.json. Math.random is stubbed only where needed (mulligan immutability check). · **Files:** new `vitest.config.ts`, new `src/lib/__tests__/gameEngine.test.ts`, `package.json` (added `test` / `test:watch` scripts)
...
**Size:** M · **Status:** ✓ SHIPPED (Session 2, 2026-06-18) — `@playwright/test` 1.61 with `playwright.config.ts` running a single `mobile-chromium` project at 390×844, plus `webServer` bootstrapping `next dev`. The `e2e/battle.spec.ts` spec walks the full transition graph: pregame → mulligan_p1 (skip) → pass_to_p2_mulligan → mulligan_p2 (skip) → pass_to_p1_initial → initial_p1 (single-select auto-confirm) → pass_to_p2_initial → initial_p2 → reveal → START BATTLE → phase_flow with both HP meters visible, and a final `scrollWidth - clientWidth ≤ 0` assertion (Wave 2.2 regression net). All selectors target the aria-label format StampBadge computes (`"${jp} (${en})"`) via regex on the EN portion so a JP rewording can't false-fail the suite. The home-page spec verifies the hero wordmark + BATTLE link entry. Verified locally: 2/2 passing. New `.github/workflows/ci.yml` runs lint → tsc → vitest → build in one `unit` job, then a `needs: unit` `e2e` job that installs Chromium and runs Playwright (with a failure-only HTML report upload as an artifact). The Pages deploy workflow is intentionally left independent for now (IMPROVEMENTS.md called this "if desired"); wiring deploy to wait on CI via `workflow_run` is a follow-up. · **Files:** new `playwright.config.ts`, new `e2e/battle.spec.ts`, new `.github/workflows/ci.yml`, `package.json` (added `e2e` / `e2e:install` scripts), `.gitignore` (added `/test-results/`, `/playwright-report/`, `/playwright/.cache/`)
...
**Size:** M · **Status:** ✓ SHIPPED (Session 2, 2026-06-18) — `sharp` 0.35 generates a 480px-wide / q70 JPEG thumb for every card scan via `scripts/make-thumbs.mjs` (idempotent overwrite, 422 files → 27 MB total under `public/cards/thumbs/`, ~6.4× smaller than the 174 MB source set). `cardAssets.ts` exposes `getLocalCardThumbPath(card)` alongside the existing `getLocalCardPath`. CardView gained a `thumb?: boolean` prop that switches the Image src between the two paths; threaded `thumb` through every render site except the gallery Spotlight (the only ≥234px site, which keeps full-res): HandDrawer fan-peek + drawer-expanded, PlayerField battle-zone + all three set-zone slots, RevealScreen P1+P2 hero cards, the bare GalleryCell in the 422-cell grid, and `CardMini` (defaults to thumb=true). A neighbor-preload `useEffect` in gallery/page.tsx fires `new window.Image().src = getLocalCardPath(neighbor)` on the two cards flanking the current Spotlight so arrow-key navigation no longer stalls 300–800 ms per step. The thumbs directory is committed alongside the existing source scans so the static export is self-contained without a prebuild step. · **Files:** new `scripts/make-thumbs.mjs`, `src/lib/cardAssets.ts`, `src/components/CardView.tsx`, `src/components/HandDrawer.tsx`, `src/components/PlayerField.tsx`, `src/components/battle/RevealScreen.tsx`, `src/app/gallery/page.tsx`, `package.json` (added `thumbs` script), new `public/cards/thumbs/` (422 JPGs)
...
**Size:** M · **Status:** ✓ SHIPPED (Session 1, 2026-06-18) — CardView wrapped in React.memo + forwardRef so the gallery grid can address the underlying button for focus restoration; new `bare?: boolean` prop suppresses attribute kanji / rarity stamp / power overlay / caption strip so the 422-cell grid renders just the bordered artwork; new GalleryCell memoized component with stable useCallback onSelect + ref-registration callback so filter toggles only re-render cells whose props actually changed. · **Files:** `src/components/CardView.tsx:33, 99-161`, `src/app/gallery/page.tsx:46-47, 84-104, 246-255`
...
**Size:** M · **Status:** ✓ SHIPPED (Session 1, 2026-06-18) — three pieces in place: (1) SfxProvider (the post-3.3 app-wide client wrapper) now wraps its children in `<MotionConfig reducedMotion="user">` so every motion/react animation honors the OS-level prefers-reduced-motion signal; (2) globals.css gained a `@media (prefers-reduced-motion: reduce)` block that neutralizes the three `.animate-*` keyframes, forces `html { scroll-behavior: auto }`, and clamps `transition-duration` / `animation-duration` to 0.01ms on every element; (3) ChronosClock uses `useReducedMotion()` to conditionally omit the SMIL `<animate>` element entirely (SMIL is immune to CSS media queries so it has to be removed from the DOM). · **Files:** `src/components/SfxProvider.tsx`, `src/app/globals.css`, `src/components/ChronosClock.tsx`
...
**Size:** M · **Status:** ✓ SHIPPED (Session 1, 2026-06-18) — six pieces: (1) BattleBoard log `<ul>` gets `aria-live="polite" aria-atomic="false" aria-relevant="additions"` so appended `<li>` entries announce; (2) PlayerField HP track gains `role="meter" aria-valuemin={0} aria-valuemax={100} aria-valuenow={hp} aria-valuetext="P1 HP: 73 of 100"`; (3) ChronosClock SVG gets `aria-hidden="true"` (text readout below is the accessible representation); (4) BattlePage now mounts a permanent SR-only `role="status" aria-live="assertive"` region whose text updates when the battle result is computed — BattleAnimationOverlay's own aria-live was dead because it mounted with its content, so the overlay is now itself `aria-hidden="true"`; (5) HandDrawer dialog gets `aria-modal="true"` + Escape-to-close + auto-focus on first card on open (deferred one rAF for the slide-up to commit) + focus-restoration to the fan trigger button on close; (6) gallery Spotlight focuses its close stamp on mount via aria-label lookup, and the page restores focus to the originating grid cell on spotlight close via a Map keyed on card.id. · **Files:** `src/components/battle/BattleBoard.tsx`, `src/components/PlayerField.tsx`, `src/components/ChronosClock.tsx`, `src/components/battle/BattleAnimationOverlay.tsx`, `src/components/HandDrawer.tsx`, `src/app/battle/page.tsx`, `src/app/gallery/page.tsx`
